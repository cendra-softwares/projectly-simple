...
interface AnalyticsTableProps {
  ...
  onSelectedRecordsChange?: (selectedRecords: Project[]) => void;
  selectedProjects: Project[];
  includeRingCharts: boolean;
  onIncludeRingChartsChange: (checked: boolean) => void;
}
...
export function AnalyticsTable({
  ...
  const handleGeneratePdf = async () => {
    ...
    // Assuming generateAnalyticsPdf is imported and available
    await generateAnalyticsPdf(selectedProjects, { includeRingCharts });
  ...
  const activeFilters = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      filters.push({
        id: "statusFilter",
        label: `Status: ${statusConfig[statusFilter].label}`,
        value: statusFilter,
        type: "status",
      });
    }
    if (financialFilter !== "all") {
      filters.push({
        id: "financialFilter",
        label: `Financial: ${financialFilter}`,
    ...
    if (minProfit !== "") {
      filters.push({
        id: "minProfit",
        label: `Min Profit: ${minProfit}`,
        value: String(minProfit),
        type: "minProfit",
      });
    }
    if (maxProfit !== "") {
      filters.push({
        id: "maxProfit",
        label: `Max Profit: ${maxProfit}`,
        value: String(maxProfit),
        type: "maxProfit",
      });
    }
    return filters;
  ...
  const handleSelectProject = (projectId: number, isSelected: boolean) => {
  ...
  const handleSelectAllProjects = (isSelected: boolean) => {
    ...
    if (isSelected) {
      sortedProjects.forEach((project) => newSelected.add(project.id));
    }
    setSelectedProjectIds(newSelected);
  };
  ...
  return (
    <Card className="animate-slide-up shadow-card">
      ...
      <CardContent>
        {/* Filters and PDF Generation Controls */}
        <div className="flex flex-col gap-4 mb-4 animate-fade-in">
          ...
          <div className="flex items-center space-x-4">
            <Button
              ...
              disabled={selectedProjectIds.size === 0 || isGeneratingPdf}
            ...
            >
              {isGeneratingPdf ? (
                ...
                <>
                  ...
                  Generate PDF Report ({selectedProjectIds.size} selected)
                </>
              )}
            </Button>
            <div className="flex items-center space-x-2">
              <Checkbox
                id="includeRingCharts"
                checked={includeRingCharts}
                onCheckedChange={onIncludeRingChartsChange}
              />
              <Label htmlFor="includeRingCharts">Include Ring Charts</Label>
            </div>
          </div>
        </div>
